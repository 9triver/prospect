...
import BpmnGroupPropertiesConfig, { type PropertiesMap } from './config';
import { resolveTypeName } from './config/TypeNameMapping';
import Modeler from 'bpmn-js/lib/Modeler';
import type { BpmnState, BpmnContext, ModdleElement } from './type';
  ...
  initModeler(options) {
    ...
    ['element.click', 'shape.added', 'element.changed'].forEach((event) => {
      BpmnStore.addEventListener(event, function (elementAction) {
        // console.log('111',event,elementAction)
