    ...
    <div class="table-responsive" v-if="outsourcingContracts && outsourcingContracts.length > 0">
      <el-table :data="outsourcingContracts" style="width: 100%" border stripe fit v-loading="isFetching">
        ...
        <el-table-column
          min-width="150px"
          show-overflow-tooltip
          prop="milestoneNode.id"
          :label="t$('jy1App.outsourcingContract.milestoneNode')"
        >
          <template #default="scope">
            <td>
              <span v-for="(milestoneNode, i) in scope.row.milestoneNodes" :key="milestoneNode.id"
                >{{ i > 0 ? ', ' : '' }}
                <router-link
                  class="form-control-static"
                  :to="{ name: 'MilestoneNodeView', params: { milestoneNodeId: milestoneNode.id } }"
                  >{{ milestoneNode.id }}</router-link
                >
              </span>
            </td>
          </template>
      ...
      <!-- <table class="table table-striped" aria-describedby="outsourcingContracts">
                ...
                <tr>
                    ...
                    <th scope="row"><span v-text="t$('jy1App.outsourcingContract.milestoneNode')"></span></th>
                    <th scope="row"></th>
                </tr>
                </thead>
                <tbody>
                <tr v-for="outsourcingContract in outsourcingContracts"
                    ...
                    <td>
                        <span v-for="(milestoneNode, i) in outsourcingContract.milestoneNodes" :key="milestoneNode.id">{{i > 0 ? ', ' : ''}}
                            <router-link class="form-control-static" :to="{name: 'MilestoneNodeView', params: {milestoneNodeId: milestoneNode.id}}">{{milestoneNode.id}}</router-link>
                        </span>
                    </td>
                    <td class="text-right">
                        <div class="btn-group">
