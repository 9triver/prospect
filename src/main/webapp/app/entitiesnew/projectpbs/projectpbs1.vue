  ...
  <div>
    ...
    <div class="table-responsive" v-if="projectpbs && projectpbs.length > 0">
      <el-table :data="projectpbs" style="width: 100%" border stripe fit v-loading="isFetching">
        ...
        <el-table-column min-width="150px" show-overflow-tooltip prop="projectwbs.id" :label="t$('jy1App.projectpbs.projectwbs')">
          <template #default="scope">
            <td>
      ...
      <!-- <table class="table table-striped" aria-describedby="projectpbs">
                ...
                <tr v-for="projectpbs in projectpbs"
                    ...
                    <td>
                        <div v-if="projectpbs.projectwbs">
                            <router-link :to="{name: 'ProjectwbsView', params: {projectwbsId: projectpbs.projectwbs.id}}">{{projectpbs.projectwbs.id}}</router-link>
                        </div>
                    </td>
                    <td>
