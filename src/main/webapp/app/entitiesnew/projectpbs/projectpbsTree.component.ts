    ...
    const details = (flatData: IProjectpbs[]) => {
      ...
      projectpbs.value.forEach(async project => {
        ...
        if (project.technicaldirector && project.technicaldirector.id) {
          try {
            const officersId = project.technicaldirector.id.toString(); 
            const res = await officersService().find(officersId);
            if (res && res.name) {
              project.technicaldirector.officersname = res.name;
            }
          } catch (error) {
        ...
        // 行政负责人——检查 project.administrativedirector.id 是否存在且不为空
        if (project.administrativedirector && project.administrativedirector.id) {
          try {
            const officersId = project.administrativedirector.id.toString() ;
            const res = await officersService().find(officersId);
            // alert(`名称是 ：${res.name}`);
            if (res && res.name) {
              project.administrativedirector.officersname = res.name;
            }
          } catch (error) {
        ...
        // 责任部门——检查 project.responsibledepartment.id 是否存在且不为空
        if (project.responsibledepartment && project.responsibledepartment.id) {
          try {
            const departmentId = project.responsibledepartment.id.toString() ; // as number 使用as语法
            const res = await departmentService().find(departmentId);
            //alert(JSON.stringify(res));
            // alert(`名称是 ：${res.departmentname}`);
            if (res && res.name) {
              project.responsibledepartment.name = res.name;
            }
          } catch (error) {
        ...
        // 相关部门——检查 project.relevantdepartment.id 是否存在且不为空
        // if (project.relevantdepartments && project.relevantdepartments.id) {
        //   try {
        //     const departmentId = project.relevantdepartment.id.toString() ; // as number 使用as语法
        //     const res = await departmentService().find(departmentId);
        //     //alert(JSON.stringify(res));
        //     // alert(`名称是 ：${res.departmentname}`);
        //     if (res && res.name) {
