/* tslint:disable max-line-length */
import { vitest } from 'vitest';
import { shallowMount, type MountingOptions } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';
import { type RouteLocation } from 'vue-router';

import DeliveryContentDetails from './delivery-content-details.vue';
import DeliveryContentService from './delivery-content.service';
import AlertService from '@/shared/alert/alert.service';

type DeliveryContentDetailsComponentType = InstanceType<typeof DeliveryContentDetails>;

let route: Partial<RouteLocation>;
const routerGoMock = vitest.fn();

vitest.mock('vue-router', () => ({
  useRoute: () => route,
  useRouter: () => ({ go: routerGoMock }),
}));

const deliveryContentSample = { id: 123 };

describe('Component Tests', () => {
  let alertService: AlertService;

  afterEach(() => {
    vitest.resetAllMocks();
  });

  describe('DeliveryContent Management Detail Component', () => {
    let deliveryContentServiceStub: SinonStubbedInstance<DeliveryContentService>;
    let mountOptions: MountingOptions<DeliveryContentDetailsComponentType>['global'];

    beforeEach(() => {
      route = {};
      deliveryContentServiceStub = sinon.createStubInstance<DeliveryContentService>(DeliveryContentService);

      alertService = new AlertService({
        i18n: { t: vitest.fn() } as any,
        bvToast: {
          toast: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          'font-awesome-icon': true,
          'router-link': true,
        },
        provide: {
          alertService,
          deliveryContentService: () => deliveryContentServiceStub,
        },
      };
    });

    describe('Navigate to details', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        deliveryContentServiceStub.find.resolves(deliveryContentSample);
        route = {
          params: {
            deliveryContentId: '' + 123,
          },
        };
        const wrapper = shallowMount(DeliveryContentDetails, { global: mountOptions });
        const comp = wrapper.vm;
        // WHEN
        await comp.$nextTick();

        // THEN
        expect(comp.deliveryContent).toMatchObject(deliveryContentSample);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        deliveryContentServiceStub.find.resolves(deliveryContentSample);
        const wrapper = shallowMount(DeliveryContentDetails, { global: mountOptions });
        const comp = wrapper.vm;
        await comp.$nextTick();

        comp.previousState();
        await comp.$nextTick();

        expect(routerGoMock).toHaveBeenCalledWith(-1);
      });
    });
  });
});
