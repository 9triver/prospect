<template>
  <div>
    <h2 id="page-heading" data-cy="SubjectHeading">
      <span v-text="t$('jy1App.subject.home.title')" id="subject-heading"></span>
      <div class="d-flex justify-content-end">
        <el-button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('jy1App.subject.home.refreshListLabel')"></span>
        </el-button>
        <router-link :to="{ name: 'SubjectCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('jy1App.subject.home.createLabel')"></span>
          </el-button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && subjects && subjects.length === 0">
      <span v-text="t$('jy1App.subject.home.notFound')"></span>
    </div>
    <div class="table-responsive" v-if="subjects && subjects.length > 0">
      <el-table :data="subjects" style="width: 100%" border stripe fit v-loading="isFetching">
        ...
        <el-table-column min-width="150px" show-overflow-tooltip prop="name" :label="t$('jy1App.subject.name')" :sortable="false">
          <template #default="scope">
            <span class="field-default">{{ scope.row.name }}</span>
          </template>
        </el-table-column>
        <el-table-column min-width="150px" show-overflow-tooltip prop="type" :label="t$('jy1App.subject.type')" :sortable="false">
          <template #default="scope">
      ...
      <!-- <table class="table table-striped" aria-describedby="subjects">
                ...
                <tr v-for="subject in subjects"
                    :key="subject.id" data-cy="entityTable">
                    <td>
                    ...
                    <td class="text-right">
                        <div class="btn-group">
                            ...
                            <b-button v-on:click="prepareRemove(subject)"
                                   variant="danger"
                                   class="btn btn-sm"
                                   data-cy="entityDeleteButton"
                                   v-b-modal.removeEntity>
