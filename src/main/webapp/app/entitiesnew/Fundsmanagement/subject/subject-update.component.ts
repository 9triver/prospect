      ...
      if (this.subject.id) {
        this.subjectService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('jy1App.subject.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.subjectService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('jy1App.subject.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
