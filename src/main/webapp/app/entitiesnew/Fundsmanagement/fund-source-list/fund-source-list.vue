  ...
  <div>
    ...
    <div class="table-responsive" v-if="fundSourceLists && fundSourceLists.length > 0">
      <el-table :data="fundSourceLists" style="width: 100%" border stripe fit v-loading="isFetching">
        ...
        <el-table-column min-width="150px" show-overflow-tooltip prop="wbsid" :label="t$('jy1App.fundSourceList.wbsid')" :sortable="false">
          <template #default="scope">
        ...
        <el-table-column
          ...
          show-overflow-tooltip
          prop="wbsname"
          :label="t$('jy1App.fundSourceList.wbsname')"
          :sortable="false"
        >
          <template #default="scope">
        ...
        <el-table-column min-width="150px" show-overflow-tooltip prop="contract.id" :label="t$('jy1App.fundSourceList.contract')">
          <template #default="scope">
            <td>
        ...
        >
          <template #default="scope">
            <td>
              <span v-for="(contractPayment, i) in scope.row.contractPayments" :key="contractPayment.id"
                >{{ i > 0 ? ', ' : '' }}
                <router-link
                  class="form-control-static"
                  :to="{ name: 'ContractPaymentView', params: { contractPaymentId: contractPayment.id } }"
                  >{{ contractPayment.id }}</router-link
                >
              </span>
            </td>
          </template>
      ...
      <!-- <table class="table table-striped" aria-describedby="fundSourceLists">
                ...
                <tr>
                    ...
                    <th scope="row"><span v-text="t$('jy1App.fundSourceList.wbsid')"></span></th>
                    <th scope="row"><span v-text="t$('jy1App.fundSourceList.wbsname')"></span></th>
                    <th scope="row"><span v-text="t$('jy1App.fundSourceList.amount')"></span></th>
                    <th scope="row"><span v-text="t$('jy1App.fundSourceList.contract')"></span></th>
                ...
                <tr v-for="fundSourceList in fundSourceLists"
                    ...
                    <td>{{fundSourceList.wbsid}}</td>
                    <td>{{fundSourceList.wbsname}}</td>
                    <td>{{fundSourceList.amount}}</td>
                    <td>
                        <div v-if="fundSourceList.contract">
                            <router-link :to="{name: 'ContractView', params: {contractId: fundSourceList.contract.id}}">{{fundSourceList.contract.id}}</router-link>
                        </div>
                    </td>
                    <td>
                    ...
                    <td>
                        <span v-for="(contractPayment, i) in fundSourceList.contractPayments" :key="contractPayment.id">{{i > 0 ? ', ' : ''}}
                            <router-link class="form-control-static" :to="{name: 'ContractPaymentView', params: {contractPaymentId: contractPayment.id}}">{{contractPayment.id}}</router-link>
                        </span>
                    </td>
                    <td class="text-right">
                        <div class="btn-group">
