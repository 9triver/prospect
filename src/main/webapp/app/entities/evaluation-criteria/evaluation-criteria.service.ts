import axios from 'axios';

import { type IEvaluationCriteria } from '@/shared/model/evaluation-criteria.model';

const baseApiUrl = 'api/evaluation-criteria';

export default class EvaluationCriteriaService {
  public find(id: number): Promise<IEvaluationCriteria> {
    return new Promise<IEvaluationCriteria>((resolve, reject) => {
      axios
        .get(`${baseApiUrl}/${id}`)
        .then(res => {
          resolve(res.data);
        })
        .catch(err => {
          reject(err);
        });
    });
  }

  public retrieve(): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      axios
        .get(baseApiUrl)
        .then(res => {
          resolve(res);
        })
        .catch(err => {
          reject(err);
        });
    });
  }

  public delete(id: number): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      axios
        .delete(`${baseApiUrl}/${id}`)
        .then(res => {
          resolve(res);
        })
        .catch(err => {
          reject(err);
        });
    });
  }

  public create(entity: IEvaluationCriteria): Promise<IEvaluationCriteria> {
    return new Promise<IEvaluationCriteria>((resolve, reject) => {
      axios
        .post(`${baseApiUrl}`, entity)
        .then(res => {
          resolve(res.data);
        })
        .catch(err => {
          reject(err);
        });
    });
  }

  public update(entity: IEvaluationCriteria): Promise<IEvaluationCriteria> {
    return new Promise<IEvaluationCriteria>((resolve, reject) => {
      axios
        .put(`${baseApiUrl}/${entity.id}`, entity)
        .then(res => {
          resolve(res.data);
        })
        .catch(err => {
          reject(err);
        });
    });
  }

  public partialUpdate(entity: IEvaluationCriteria): Promise<IEvaluationCriteria> {
    return new Promise<IEvaluationCriteria>((resolve, reject) => {
      axios
        .patch(`${baseApiUrl}/${entity.id}`, entity)
        .then(res => {
          resolve(res.data);
        })
        .catch(err => {
          reject(err);
        });
    });
  }
}
