/* tslint:disable max-line-length */
import { vitest } from 'vitest';
import { shallowMount, type MountingOptions } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';
import { type RouteLocation } from 'vue-router';

import ContractDetails from './contract-details.vue';
import ContractService from './contract.service';
import AlertService from '@/shared/alert/alert.service';

type ContractDetailsComponentType = InstanceType<typeof ContractDetails>;

let route: Partial<RouteLocation>;
const routerGoMock = vitest.fn();

vitest.mock('vue-router', () => ({
  useRoute: () => route,
  useRouter: () => ({ go: routerGoMock }),
}));

const contractSample = { id: 'ABC' };

describe('Component Tests', () => {
  let alertService: AlertService;

  afterEach(() => {
    vitest.resetAllMocks();
  });

  describe('Contract Management Detail Component', () => {
    let contractServiceStub: SinonStubbedInstance<ContractService>;
    let mountOptions: MountingOptions<ContractDetailsComponentType>['global'];

    beforeEach(() => {
      route = {};
      contractServiceStub = sinon.createStubInstance<ContractService>(ContractService);

      alertService = new AlertService({
        i18n: { t: vitest.fn() } as any,
        bvToast: {
          toast: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          'font-awesome-icon': true,
          'router-link': true,
        },
        provide: {
          alertService,
          contractService: () => contractServiceStub,
        },
      };
    });

    describe('Navigate to details', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        contractServiceStub.find.resolves(contractSample);
        route = {
          params: {
            contractId: '' + 'ABC',
          },
        };
        const wrapper = shallowMount(ContractDetails, { global: mountOptions });
        const comp = wrapper.vm;
        // WHEN
        await comp.$nextTick();

        // THEN
        expect(comp.contract).toMatchObject(contractSample);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        contractServiceStub.find.resolves(contractSample);
        const wrapper = shallowMount(ContractDetails, { global: mountOptions });
        const comp = wrapper.vm;
        await comp.$nextTick();

        comp.previousState();
        await comp.$nextTick();

        expect(routerGoMock).toHaveBeenCalledWith(-1);
      });
    });
  });
});
