  ...
  <div>
    ...
    <div class="table-responsive" v-if="projectwbs && projectwbs.length > 0">
      <el-table :data="projectwbs" style="width: 100%" border stripe fit v-loading="isFetching">
        ...
        <el-table-column min-width="150px" show-overflow-tooltip prop="projectpbs.id" :label="t$('jy1App.projectwbs.projectpbs')">
          <template #default="scope">
            <td>
              <div v-if="scope.row.projectpbs">
                <router-link :to="{ name: 'ProjectpbsView', params: { projectpbsId: scope.row.projectpbs.id } }">{{
                  scope.row.projectpbs.id
                }}</router-link>
              </div>
            </td>
          </template>
                ...
                <tr v-for="projectwbs in projectwbs"
                    ...
                    <td>
                        <div v-if="projectwbs.projectpbs">
                            <router-link :to="{name: 'ProjectpbsView', params: {projectpbsId: projectwbs.projectpbs.id}}">{{projectwbs.projectpbs.id}}</router-link>
                        </div>
                    </td>
                    <td>
                    ...
                    <td>
                        <span v-for="(workbag, i) in projectwbs.workbags" :key="workbag.id">{{i > 0 ? ', ' : ''}}
