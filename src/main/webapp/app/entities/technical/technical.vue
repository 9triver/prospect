    ...
    <div class="table-responsive" v-if="technicals && technicals.length > 0">
      <el-table :data="technicals" style="width: 100%" border stripe fit v-loading="isFetching">
        ...
        <el-table-column
          ...
          prop="description"
          :label="t$('jy1App.technical.description')"
          :sortable="false"
        >
          <template #default="scope">
        ...
        <el-table-column
          ...
          show-overflow-tooltip
          prop="starttime"
          :label="t$('jy1App.technical.starttime')"
          :sortable="true"
        >
          <template #default="scope">
        ...
        <el-table-column min-width="150px" show-overflow-tooltip prop="endtime" :label="t$('jy1App.technical.endtime')" :sortable="true">
          <template #default="scope">
            <span class="field-default">{{ scope.row.endtime }}</span>
          </template>
        </el-table-column>
        <el-table-column min-width="150px" show-overflow-tooltip prop="creatorid.id" :label="t$('jy1App.technical.creatorid')">
          <template #default="scope">
            <td>
        ...
        <el-table-column min-width="150px" show-overflow-tooltip prop="auditorid.id" :label="t$('jy1App.technical.auditorid')">
          <template #default="scope">
            <td>
        ...
        <el-table-column min-width="150px" show-overflow-tooltip prop="projectwbs.id" :label="t$('jy1App.technical.projectwbs')">
          <template #default="scope">
            <td>
        ...
        <el-table-column min-width="150px" show-overflow-tooltip label="操作">
          <template #default="scope">
            <td class="text-right">
              <div class="btn-group">
                <router-link :to="{ name: 'TechnicalView', params: { technicalId: scope.row.id } }" custom v-slot="{ navigate }">
                  <button @click="navigate" class="btn btn-info btn-sm details" data-cy="entityDetailsButton">
                    <font-awesome-icon icon="eye"></font-awesome-icon>
                    <span class="d-none d-md-inline" v-text="t$('entity.action.view')"></span>
                  </button>
                </router-link>
                <router-link :to="{ name: 'TechnicalEdit', params: { technicalId: scope.row.id } }" custom v-slot="{ navigate }">
      ...
      <!-- <table class="table table-striped" aria-describedby="technicals">
                ...
                <tr v-for="technical in technicals"
                    :key="technical.id" data-cy="entityTable">
                    <td>
                    ...
                    <td>
                        <div v-if="technical.creatorid">
                            <router-link :to="{name: 'OfficersView', params: {officersId: technical.creatorid.id}}">{{technical.creatorid.id}}</router-link>
                        </div>
                    </td>
                    <td>
                        <div v-if="technical.auditorid">
                            <router-link :to="{name: 'OfficersView', params: {officersId: technical.auditorid.id}}">{{technical.auditorid.id}}</router-link>
                        </div>
                    </td>
                    <td>
                        <span v-for="(projectwbs, i) in technical.projectwbs" :key="projectwbs.id">{{i > 0 ? ', ' : ''}}
                    ...
                    <td class="text-right">
                        <div class="btn-group">
                            ...
                            <b-button v-on:click="prepareRemove(technical)"
                                   variant="danger"
                                   class="btn btn-sm"
                                   data-cy="entityDeleteButton"
                                   v-b-modal.removeEntity>
