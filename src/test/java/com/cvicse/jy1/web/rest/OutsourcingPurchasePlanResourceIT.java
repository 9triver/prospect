...
import static com.cvicse.jy1.domain.OutsourcingPurchasePlanAsserts.*;
...
class OutsourcingPurchasePlanResourceIT {
    ...
    @Mock
    private OutsourcingPurchasePlanRepository outsourcingPurchasePlanRepositoryMock;

    @Mock
    private OutsourcingPurchasePlanService outsourcingPurchasePlanServiceMock;
    ...
    private OutsourcingPurchasePlan insertedOutsourcingPurchasePlan;
    ...
    @AfterEach
    public void cleanup() {
        if (insertedOutsourcingPurchasePlan != null) {
            outsourcingPurchasePlanRepository.delete(insertedOutsourcingPurchasePlan);
            insertedOutsourcingPurchasePlan = null;
        }
    }
    ...
    void createOutsourcingPurchasePlan() throws Exception {
        ...
        insertedOutsourcingPurchasePlan = returnedOutsourcingPurchasePlan;
    }
    ...
    void getAllOutsourcingPurchasePlans() throws Exception {
        ...
        insertedOutsourcingPurchasePlan = outsourcingPurchasePlanRepository.saveAndFlush(outsourcingPurchasePlan);
    ...
    void getAllOutsourcingPurchasePlansWithEagerRelationshipsIsEnabled() throws Exception {
        when(outsourcingPurchasePlanServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restOutsourcingPurchasePlanMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(outsourcingPurchasePlanServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllOutsourcingPurchasePlansWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(outsourcingPurchasePlanServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restOutsourcingPurchasePlanMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(outsourcingPurchasePlanRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
